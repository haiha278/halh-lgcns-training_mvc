...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public UpdateUserDTO getUserInfoToUpdateByUsername(String username) {
        SqlSession session = MyBatisUtil.getSqlSessionFactory().openSession();
        UserMapper user_mapper = session.getMapper(UserMapper.class);
        Optional<User> optionalUser = user_mapper.findByUsername(username);
        if (optionalUser.isPresent()) {
            User user = optionalUser.get();
            UpdateUserDTO updateUserDTO = (UpdateUserDTO) mapper.mapToDTO(user, UpdateUserDTO.class);
    ...
    @Override
    public int updateByUsername(UpdateUserDTO updateUserDTO) {
        SqlSession session = MyBatisUtil.getSqlSessionFactory().openSession();
        UserMapper user_mapper = session.getMapper(UserMapper.class);

        User user = (User) mapper.mapToEntity(updateUserDTO, UpdateUserDTO.class);

        int rowEffected = user_mapper.updateByUsername(user);
